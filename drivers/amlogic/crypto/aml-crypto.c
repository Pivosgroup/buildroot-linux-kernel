...
static void    write_aes_reg(AES_KEY * aes_key)
{
    unsigned int Nr = aes_key->rounds;
    unsigned rk_length = 4*(Nr+1);
    unsigned i;
    unsigned int * rk_ptr = aes_key->rd_key;

    Wr(NDMA_AES_CONTROL, Rd(NDMA_AES_CONTROL) | (1 << 31)); // reset the RK fifo write pointer
    for(i=0;i<rk_length;i++)
    {
            Wr(NDMA_AES_RK_FIFO,*(rk_ptr+i));
    }
    
    Wr(NDMA_AES_CONTROL, (Rd(NDMA_AES_CONTROL) & ~(0xf << 0)) | (Nr << 0));
...
aml_ecb_aes_encrypt(struct blkcipher_desc *desc,
		  ...
		  unsigned int nbytes)
{ 
        printk("This chip don't support aes encrypto!\n");
        return 1; 
}
