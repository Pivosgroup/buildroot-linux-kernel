...
static void vavs_vf_put(vframe_t *);

static const char vavs_dec_id[] = "vavs-dev";

static const struct vframe_provider_s vavs_vf_provider = {
        .peek = vavs_vf_peek,
        .get = vavs_vf_get,
        .put = vavs_vf_put,
};
...
static void vavs_vf_put(vframe_t *vf)
{
        INCPTR(putting_ptr);
}
...
static void vavs_put_timer_func(unsigned long arg)
{
        ...
        if ( READ_MPEG_REG(AVS_SOS_COUNT) > 10 )
        {
                ...
                vavs_local_init();
                vavs_prot_init();
                vf_reg_provider(&vavs_vf_provider);
                amvdec_start();
        }
#endif
