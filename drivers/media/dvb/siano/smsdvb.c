			       ...
			       struct dvb_frontend_parameters *fep)
{
	...
	//fy>>
        ...
        printk("YFENG(%s)!!!!!!!!!\n"
                "fep->frequency = %d\n"
                "fep->u.ofdm.bandwidth = %d\n"
                "fep->u.ofdm.code_rate_HP = %d\n"
                "fep->u.ofdm.code_rate_LP = %d\n"
                "fep->u.ofdm.constellation = %d\n"
                "fep->u.ofdm.transmission_mode = %d\n"
                "fep->u.ofdm.guard_interval = %d\n"
                "fep->u.ofdm.hierarchy_information = %d\n"
                "fep->inversion = %d\n",
                        __func__, fep->frequency, fep->u.ofdm.bandwidth, fep->u.ofdm.code_rate_HP, fep->u.ofdm.code_rate_LP,
                        fep->u.ofdm.constellation, fep->u.ofdm.transmission_mode, fep->u.ofdm.guard_interval,
                        fep->u.ofdm.hierarchy_information, fep->inversion);
        #endif
      //fy<<
	switch (client->coredev->mode) {
		...
		case DEVICE_MODE_ISDBT_BDA:
			val=smsdvb_tune_isdbt(client, fep);
			msleep(2000);
			return val;
		default:
