...
#if 0 //fy>> #ifdef REQUEST_FIRMWARE_SUPPORTED
...
#else
	char* fw_filename = smscore_get_fw_filename(coredev, mode, lookup);
	sms_info("look firmware name =%s in const buffer",fw_filename);
	const unsigned char * pfw;
	if (!strcmp(fw_filename,"dvb_nova_12mhz_b0.inp")) {
	    pfw = inp_dvb_nova_12mhz_b0;//dvb_nova_12mhz_b0;//
	    fw_buf_size = LEN_INP_DVB_NOVA_12MHZ_B0;//dvb_nova_12mhz_b0_size;//
	} else if (!strcmp(fw_filename,"isdbt_rio.inp")) {
	    pfw = inp_isdbt_rio;
	    fw_buf_size = LEN_INP_ISDBT_RIO;
	} else {
	    sms_err("load firmware failed %s", fw_filename);
	    return -ENOENT;
	}
